...
Como não temos acesso a dados reais de um hospital, foi utilizado dois tipos de dados
...
para facilitar reprodução local caso necessário:
...
Utilizei o **Qwen2.5-1.5B-Instruct**, um modelo relativamente pequeno (1,5 bilhão
de parâmetros) e de uso livre. A escolha foi baseada no poder da minha máquina local, de maneira resumida,
rodou em uma GPU de notebook (GTX 1650, com 4 GB de memória de vídeo),
então eu precisava de um modelo que coubesse nesse hardware.
...
- **LoRA (Low-Rank Adaptation)**: em vez de re-treinar o modelo inteiro (1,5
  bilhão de parâmetros), treinei apenas um pequeno "adaptador" extra, com
  cerca de 18 milhões de parâmetros. O modelo original fica
  intocado, daí esse adaptador é o que aprende o conhecimento novo.

Essa combinação (quantização + LoRA) tem um termo chamado **QLoRA**.

### O dataset e uma descoberta no caminho

A partir do MedQuAD, que tem mais de 47 mil perguntas e respostas médicas
públicas. Depois de limpar duplicatas e respostas vazias ou curtas demais,
fiquei com um conjunto bem menor. Nessa limpeza, percebi algo interessante:
...
pouco útil, então foi removido essas respostas antes do treino final.
...
do conteúdo mesmo com uma amostra menor.

### O treinamento em números

- **1.425 exemplos de treino** e 75 de validação (amostra estratificada).
- **1 época** de treino (teve 179 passos), levando cerca de **2h47min** para finalizar.
- Para não crashar o computador (isso aconteceu uma vez, durante os testes
  iniciais), limitei no script de treino o uso de memória da GPU a 80% do total.
- A "loss" (uma medida de quanto o modelo ainda erra — quanto menor, melhor)
  caiu de forma consistente durante o treino, e terminanou em 1,35 no
  conjunto de treino e **1,01** no conjunto de validação (dados que o modelo
  não viu durante o treino). A modo de comparação, a primeira versão do treino
...
6. **Redige um laudo clínico formatado**, reunindo diagnóstico, exames,
   ...
   estrutura fixa (seguindo um modelo de referência criado em
   ...
   é exportado para PDF em `outputs/reports/`.
7. **Aplica guardrails de segurança**: o sistema verifica se a resposta tem
   uma prescrição direta e imperativa (por exemplo, "tome 500mg de X") e,
   ...
   qualquer tomada de decisão do médico, dessa forma o assistente nunca substitui o julgamento do
...
8. **Audita tudo**: cada pergunta, resposta, fonte usada e metadado é salvo em
   ...
   perguntado, o que foi respondido e com base em que.

Esse fluxo de decisão acima é orquestrado com **LangGraph**, uma
...
Para reduzir as respostas fora do contexto, cada prontuário também
...
- **Modelo original (sem fine-tuning):** deu uma resposta genérica de bom
  ...
  de referência.
- **Modelo fine-tuned (primeira versão, com o dataset ainda "precário"):** piorou
...
Também foi testado uma pergunta sobre sepse dentro do fluxo completo (com busca
...
Durante a validação dos casos sorteados, encontrei um risco importante:
...
passou a declarar seus protocolos autorizados. O fluxo também separa
alertas informativos de alertas críticos e vincula a conduta principal no
...
português, dentro de um prompt mais longo), percebi que o modelo às vezes
...
que deixo registrada aqui. Chegamos a testar se bastaria instruir explicitamente no
...
de qualidade real em cenários diretos.
